...
const {
  laporanSppSiswaRepository,
  kekuranganPembayaranRepository,
  laporanSppMysqlRepository,
  kekuranganPembayaranMysqlRepository,
} = require("../repository");
...
async function getDataSpp(req) {
  try {
    ...
    const getDataSppTransaksi = laporanSppMysqlRepository.getDataSpp(
      bulan_bayar,
      tahun_bayar,
      kelas,
      page,
...
async function getDataSppByNisn(req) {
  try {
    const { nisn, kode_bayar, kelas } = req.query;
    console.log("payload: ", nisn);
    const getDataSppByNisns = await laporanSppMysqlRepository.getDataSppByNisn(
      nisn,
      kode_bayar
    );
    ...
    if (getDataSppByNisns) {
      nilaiKekuranganSiswa = await laporanSppMysqlRepository.nilaiKekuranganPembayaran(
        nisn,
        kelas,
        kode_bayar
      );
...
async function getBulanBelumBayar(req) {
  try {
    ...
    const getDataBulan = laporanSppMysqlRepository.getBulanBelumBayar(
      nisn,
      kelas,
      tahunAjaran
    );
...
async function inputPembayaran(req) {
  ...
  try {
    const findKodePembayaran = await laporanSppMysqlRepository.getKodePembayaran(
      kelas,
      tahun_ajaran
    );
    ...
    for (const datas of bulan) {
      ...
      const validasiBulanBayar = await laporanSppMysqlRepository.validasiBulanBayar(
        nisn,
        bulan_bayar,
        tahun_ajaran
      );
      if (validasiBulanBayar !== 0) {
        ...
        const dataKurangBayar = await laporanSppMysqlRepository.getDataKekurangan(
          nisn,
          kode_bayar
        );

        const countData = await laporanSppMysqlRepository.getCountKekuranganPembayaranSiswa(
          nisn,
          kode_bayar
        );
        // insert table bayar spp
        await laporanSppMysqlRepository.insertPaymentSiswa(
          nama,
          kelas,
          nisn,
          kode_bayar,
        ...
        if (countData === 0) {
          const perhitungan =
            findKodePembayaran[0].nominal_total - nominal_bayar;
          await kekuranganPembayaranMysqlRepository.insertTableKekuranganPembayaranSiswa(
            nama,
            kelas,
            nisn,
            kode_bayar,
          ...
          await kekuranganPembayaranMysqlRepository.insertTableKekuranganPembayaranSiswa(
            ...
            kode_bayar,
            "SPP",
            perhitunganNominalAkhir,
            petugas_input
          );

          //get data keuangan sekolah
          const getDataKeuanganSekolah = await laporanSppMysqlRepository.getDataKekuaranganPemasukan(
            kode_bayar
          );
          const uang_sisa_kekurangan = parseFloat(
            getDataKeuanganSekolah.kekurangan_pemasukan
          );
          ...
          await laporanSppMysqlRepository.updateDataKekuranganPemasukan(
            kelas,
            kode_bayar,
            "SPP",
            perhitungan_uang_masuk,
            perhitungan_sisa_kekurangan
          );
        }
      }
    }
...
async function getHistoryPembayaranSppNew(req) {
  try {
    ...
    const dataHistoryPembayaranSppNew = laporanSppMysqlRepository.getHistoryPembayaranSppNew(
      page,
      pageSize
    );
...
async function getJenisPembayaran(req) {
  ...
  try {
    const jenisPembayaran = laporanSppMysqlRepository.jenisPembayaran(
      tahun_ajaran
    );
...
async function insertJenisPembayaran(req) {
  ...
  try {
    const data = await laporanSppMysqlRepository.insertJenisPembayaran(
      kode_pembayaran,
      jenis_transaksi,
      kelas,
      nominal_bulan,
...
async function getJeninsPembayaranForNominal(req) {
  ...
  try {
    data = await laporanSppMysqlRepository.getJeninsPembayaranForNominal(
      kelas,
      tahun_ajaran
    );
    return data;
