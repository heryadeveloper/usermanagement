...
async function insertPaymentSiswa(
  nama,
  kelas,
  nisn,
  kode_bayar,
...
async function getDataKekurangan(nisn, kode_bayar) {
  try {
    const dataKekurangan = await db.kekurangan_pembayaran_siswa.findAll({
      ...
      attributes: [
        [
          db.sequelize.fn("MIN", db.sequelize.col("kekurangan_pembayaran")),
          "nilai_terkecil",
        ],
      ],
      raw: true,
...
async function updateDataKekuranganPemasukan(
  kelas,
  kode_pembayaran,
  jenis_pembayaran,
  uang_masuk,
...
async function nilaiKekuranganPembayaran(nisn, kelas, kode_bayar) {
  try {
    console.log(nisn, kelas, kode_bayar);
    const query = `select 
                    jp.nominal_bulan,
                    jp.kode_pembayaran,
                    jp.nominal_total,
                    ...
                    where ps.nisn = :nisn
                    and ps.kode_bayar = :kode_bayar
                    and ps.kelas = :kelas
                    group by jp.nominal_bulan, jp.kode_pembayaran, jp.nominal_total, ps.nama_siswa`;
    const responseData = await db.sequelize.query(query, {
      replacements: { nisn, kelas, kode_bayar },
      type: db.Sequelize.QueryTypes.SELECT,
    });
...
async function jenisPembayaranPaymentUsingJenisTransaksi(
  tahun_ajaran,
  jenis_transaksi
) {
  try {
...
async function insertJenisPembayaran(
  kode_pembayaran,
  jenis_transaksi,
  kelas,
  nominal_bulan,
